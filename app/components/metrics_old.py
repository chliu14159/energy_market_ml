...
def calculate_customer_metrics(df, customer_name=None):
    """
    Calculate customer-specific metrics (adapted for regions)
    
    Args:
        df (pd.DataFrame): Analytics dataframe
        customer_name (str): Region ID to filter by
    ...
    if customer_name:
        region_data = df[df['REGIONID'] == customer_name]
    else:
        region_data = df
